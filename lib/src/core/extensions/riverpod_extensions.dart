...
extension RefAsListenable on Ref {
  ValueListenable<T> asListenable<T>(ProviderBase<T> provider) {
    ...
    onResume(() {
      // WHY: Riverpod 3 forbids `Ref` use inside lifecycle callbacks; the
      // subscription can read the current value without touching `Ref`.
      final latestValue = providerSubscription.read();
